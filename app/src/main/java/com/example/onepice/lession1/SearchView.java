...
public class SearchView extends EditText {
    ...
    private void DrawSearchIcon(Canvas canvas) {
        if (this.getText().toString().length() == 0) {
            ...
            canvas.drawText("请输入要搜索的eos账号名", getScrollX() + searchSize + 8, getScrollY() + (getHeight() - (getHeight() - textHeight) / 2) - paint.getFontMetrics().bottom - dy, paint);
            canvas.restore();
        }
    }
